...
function getPosts() {
	$db = getData();
	$posts = $db->query('select BLOG_POST_ID as id, BLOG_POST_DATE as date,'
...
function getPost($id_Post) {
	$db = getData();
	$post = $db->prepare('select BLOG_POST_ID as id, BLOG_POST_DATE as date,'
	. ' BLOG_POST_PHOTO as photo, BLOG_POST_TITLE as title, BLOG_POST_PREVIEW as preview, BLOG_POST_DATA as data from BLOG_POST'
...
function setPost($id_post, $data, $blog='BLOG_POST') {
	
	if($id_post==0) {
		$columns = implode(", ", array_keys($data));
		$values  = implode(", ", array_values($data));
		$db->prepare('INSERT INTO :blog (:columns) VALUES (:values)',array('blog'=>$blog, 'columns'=>$columns, 'values'=>$values));
	} else {
		$parts = array();
		...
		$db->prepare('UPDATE :blog (:columns) VALUES (:values) WHERE BLOG_POST_ID = :id',array('values'=>$values, 'blog'=>$blog, 'id' => $id_post));
	}
}
...
function getPage($id_Page) {
  ...
  if ($post->rowCount() == 1)
   ...
   throw new Exception("No post for the ID : " . $id_Page);*/
	...
	return array('id' => 1, 'title' => 'Blog and CV', 'picture' => 'uploads/logo.png', 'subtitle' => 'Devloper PHP7/Symphony2', nav => array('title' => 'Blog and CV', 'menu' => 'Menu'), 'cv' => $cv, 'blog' => $blog, 'footer' => $footer);
}
